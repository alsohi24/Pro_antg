	...
	protected void actionPerformedBtnRegistrar(ActionEvent arg0) {
		if (leerCodConsumo() == -1) {
			...
			if (jtblConsumo.getRowCount() != 0) {
				...
				if (msj == 0) {
					ac.adicionar(new Consumo(leerCodConsumo(), leerCodPaciente(), Fecha.getFecha(cboDia, cboMes, cboAno), 0,
							total));
					ac.grabarConsumo();
					...
					for (int i = 0; i < jtblConsumo.getRowCount(); i++) {
						...
						adet.adicionar(
								new DetalleConsumo(leerCodConsumo(), codProducto, cantidad, precioUnitario, subtotal));
					}
