...
public class DlgControlDePagos extends JDialog implements ActionListener {
	...
	void guardarFactura() {
		...
		if (!codDet.isEmpty()) {
			...
			adet.adicionar(new DetalleConsumo(codDet.get(codDet.size() - 1), nroCama, cantDias, precioxDia,
					cantDias * precioxDia));
		}
		else {
			acon.adicionar(
					new Consumo(acon.codigoCorrelativo(), leerCodigoPaciente(), fecha, 1, cantDias * precioxDia));
			adet.adicionar(new DetalleConsumo(acon.obtener(acon.tamaño() - 1).getCodConsumo(), nroCama, cantDias,
